/**
 *
 * This the hybrid programme in multiple style on Cirrus.
...
int main(int argc, char **argv)
{
  ...
  **/
   ...
   * of which size is 5000x18 dynamically
   * 5000 represents the number of iterations
   * 18 is the number of threads on each rank. If users change the number of nodes, ranks or threads,
  ...
  **/
  ...
  time_iter_thread = (double **)arraymalloc2d(5000,18,sizeof(double));
  time_comm_thread = (double **)arraymalloc2d(5000,18,sizeof(double));
  time_comp_thread = (double **)arraymalloc2d(5000,18,sizeof(double));
...
#pragma omp parallel default (none) shared(time_iter_thread,time_comm_thread,time_comp_thread,zettmp,psitmp,zet,psi,n,numiter,irrotational,lm,re,comm,printfreq,rank,checkerr) private(i,j)
  ...
  **/
  //printf("rank = %d\n",rank);
  for(i=0;i<5000;i++) {
     MPI_Gather(time_iter_thread[i],18,MPI_DOUBLE,time_iter[i],18,MPI_DOUBLE,0,comm);
     MPI_Gather(time_comp_thread[i],18,MPI_DOUBLE,time_comp[i],18,MPI_DOUBLE,0,comm);
     MPI_Gather(time_comm_thread[i],18,MPI_DOUBLE,time_comm[i],18,MPI_DOUBLE,0,comm);
  }
